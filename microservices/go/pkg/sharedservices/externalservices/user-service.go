...
type ExtUserService interface {
	...
	GetById(ctx context.Context, id string) single.Single[userdtos.UserDto]
}
...
func (u ExtUserServiceImpl) GetById(ctx context.Context, id string) single.Single[userdtos.UserDto] {
	...
	return single.Map(userReplySrc, func(reply *userpb.UserReply) userdtos.UserDto {
		...
		grpcmappers.MapUserReplyToUserDto(reply, &dto)
		return dto
	})
}
